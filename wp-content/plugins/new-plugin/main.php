...
function createTable(){
    global $wpdb;
    $qr=$wpdb->query("CREATE TABLE IF NOT EXISTS `wp_contact_plugin_test` ( `id` INT NOT NULL AUTO_INCREMENT , `email` TEXT NOT NULL , `name` TEXT NOT NULL , `subject` TEXT NOT NULL , `message` TEXT NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;");
}
function deleteTable(){
    global $wpdb;
    $qr=$wpdb->query("DROP TABLE IF EXISTS `wp_contact_plugin_test`;");
}
function saveDataToTable($email,$name,$message,$subject){
    global $wpdb;
    $sr=$qr=$wpdb->query("INSERT INTO `wp_contact_plugin_test` (`id`, `email`, `name`, `subject`, `message`) VALUES (NULL, '{$email}', '{$name}', '{$subject}', '{$message}');");
}
